...
# Crypto package safe import
try:
    from Crypto.Cipher import AES
    from Crypto.Util.Padding import pad
except ModuleNotFoundError:
    print("PyCryptodome package missing, check requirements.txt")
...
# 🌐 Zeabur direct root route handler
...
def direct_like_handler():
    ...
    if not uid_str and not key:
        return jsonify({
            "status": "online",
            "message": "Free Fire Likes API is running successfully on Zeabur!"
        })

    if not uid_str or not region or not key:
        return jsonify({"error": "Missing parameters.", "status": 0}), 400
    ...
    if not tokens:
        return jsonify({"error": f"No tokens found for region {region}", "status": 0}), 500
    ...
    async def process_request():
        ...
        try:
            ...
            return jsonify({"error": "Protobuf/Encryption failed", "status": 0}), 500
                    ...
                    async with session.get(url2, headers=headers, timeout=5) as resp2:
                        if resp2.status == 200:
                            ...
                            try:
                                ...
                                if info_obj:
                                    after_like = getattr(info_obj, 'Likes', after_like)
...
if __name__ == '__main__':
    # Default port configuration for Zeabur hosting
